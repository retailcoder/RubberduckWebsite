...
namespace RubberduckServices
{
    ...
    {
        public async Task<string[]> IndentAsync(string code, IIndenterSettings settings)
        {

            return await Task.Run(() =>
            {
                var adapter = new IndenterSettingsAdapter(settings);
                var indenter = new Rubberduck.SmartIndenter.Indenter(null, () => adapter);
                return indenter.Indent(code).ToArray();
            });
        }
    }
}
